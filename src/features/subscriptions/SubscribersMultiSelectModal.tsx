// Общий мультивыбор аккаунтов из подписчиков текущего пользователя.
// Используется для организаторов события и менеджеров организации.
...
export interface SubscribersMultiSelectModalProps {
  currentAccountId: string;
  existingIds: ReadonlySet<string>;
  /** Бейдж у уже добавленных (например «Организатор» / «В команде») */
  existingBadge: string;
  title: string;
  ariaLabel?: string;
  /** Базовый текст кнопки подтверждения без счётчика */
  confirmLabel?: string;
  onClose: () => void;
  /** Режим «вернуть выбор» родителю (без сохранения внутри модалки) */
  onConfirm?: (accountIds: string[]) => void;
  /** Режим «сохранить внутри» + экран успеха */
...
export function SubscribersMultiSelectModal({
  currentAccountId,
  existingIds,
  existingBadge,
  title,
...
}: SubscribersMultiSelectModalProps) {
  ...
  const selectable = useMemo(
    () => subscribers.filter(s => !existingIds.has(s.account.id)),
    [subscribers, existingIds],
  );

  const toggleSelect = (id: string) => {
    if (existingIds.has(id) || saving || done) return;
    setSelected(prev => {
  ...
  return createPortal(
    <>
      ...
      <div className={styles.modal} role="dialog" aria-modal aria-label={ariaLabel ?? title}>
        ...
        {!loading && selectable.length > 0 && !done && (
        ...
        <div className={styles.list}>
          {loading ? (
            ...
            <>
              {subscribers.map(s => {
                const isExisting = existingIds.has(s.account.id);
                ...
                return (
                  ...
                  >
                    ...
                    {isExisting && <span className={styles.orgBadge}>{existingBadge}</span>}
                  </div>
                );
              })}

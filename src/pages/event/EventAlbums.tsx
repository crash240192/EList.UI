...
function SpinnerImage({ fileId, alt, className }: { fileId: string; alt?: string; className?: string }) {
  ...
  return (
    <div className={styles.imgWrap} style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      {error ? (
        ...
        <AuthImage fileId={fileId} alt={alt ?? ''} className={`${className ?? styles.img} ${loaded ? styles.imgLoaded : styles.imgHidden}`}
    ...
    <div className={`${styles.viewerBackdrop} ${fullscreen ? styles.viewerFs : ''}`} onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className={styles.viewer}>
        ...
        <div className={styles.viewerMain}
          ...
          {cur && <SpinnerImage fileId={cur.fileId} alt={`Фото ${idx + 1}`} className={styles.viewerImg} />}
          {idx > 0 && (
